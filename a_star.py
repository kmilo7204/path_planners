...
class AStar:
    ...
    def init_open(self): # Open list initialization, this could be more clear.
        """
        Initialize the open list with the initial position or
        start node
        """
        # Coste real del camino recorrido desde el nodo inicial g_cost
        g_cost = 0 # What is g_cost
        # Valor o funcion que representa el valor heuristico de distancia.
        distance = self.calculate_distance(self.start) # What means distance (Heuristic) ?
        # Funcion de evaluacion f_cost
    ...
    # This could be mapped to the heuristic function directly (Renamed)
    ...
    def search_astar(self):
        ...
        # Infite values for all the nodes in intialization
        ...
        # Set contianing the path - Because we do not want to have duplicated nodes here
        prior_set = {}

        # List for the visited nodes
        visited_ls = []
        visited_ls.append(self.start) # Initial node is Visited hence we append it

        # Initial G and F scores considering the heuristics
        g_score[self.start[0]][self.start[1]] = 0 # No cost of moving to the initial node
        # Full cost (evaluation function)
            ...
            if (len(self.open_ls) == 0):
                ...
                for action in neighbors_ls:
                    ...
                    if ((new_x >= 0) and (new_x < len(self.grid)) and (new_y >= 0) and (new_y < len(self.grid[0])) and (self.grid[new_x][new_y] == 0)):
                        ...
                        prov_g = current[0] + 1 # Here the cost of motion os only 1, it could be different
