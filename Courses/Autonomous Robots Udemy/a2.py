...
class AStarSearch:
    ...
    def get_possible_moves(self):
        """
            1. Check for the validity of the move.

            2. Check if the move is in the explored
            or in the not explored list, in case
            it is not, we will add the node to the
            not explored list, but we will update
            the cost with the current depth + 1 +
            the heuristic metric.

            We aslo update the Heuristics grid for
            each move.
        """
        ...
        # Current position is marked as explored, since
        # we have found the next movements.
        self.explored[self.pos_str] = self.pos_depth
        return True

    def goal_found(self):
        """
            Check if the goal is in the not
            explored list.

            In case is it we get the position
            and its depth and remove it from
            the not explored list.

            Finally we add the element the node
            into the final path.
        """
    ...
    def explore_next_move(self):
        """
            ...
            move based on the lowest cost.

            Take the not explored list and
            sort it from the lower to the
            largest cost.

            Select the lower cost and remove
            it from the not explored list.
        """
        ...
        # Take the Position and its Depth
    ...
    def heuristic(self, move):
        distance = np.sqrt((move[0] - goal[0])**2 + (move[1] - goal[1])**2)
    ...
    def generate_potential_moves(self, pos):
        ...
        # We will add the diagonal moves
