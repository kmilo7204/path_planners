...
class BreadthFirstSearch:
    ...
    def get_possible_moves(self):
        """
            This function returns the next potential moves up, down,
            left and right
        """
        ...
        for move in potential_moves:
            ...
            """
                This function asserts the movement limits
                so we avoid moving outside the grid
            """
            ...
            if (str(move) not in self.explored) and (str(move) not in self.not_explored):
                """
                    Here we are adding one more step (Depth)
                """
        ...
        """
            Add the current location (position) to the explored liost
        """
    ...
    def explore_next_move(self):
        ...
        # We got the lowest depth here and we can randomly select the next move
        self.pos_str = sorted_not_explored[0]
        self.pos = self.string_to_array(self.pos_str)
        # Remove the selected node from the not explored list, since it will be explored
        self.pos_depth = self.not_explored.pop(sorted_not_explored[0])
        # Write depth of next move onto path.
        path[self.pos[0], self.pos[1]] = self.pos_depth
        return True
