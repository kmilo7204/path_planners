...
class AStarSearch:
    ...
    def get_possible_moves(self):
        ...
        for move in potential_moves:
            if not self.valid_move(move):
                continue
            # If move has not been explored as is not in the queue to be explored so add it
    ...
    def explore_next_move(self):
        # Determine next move to explore. Sort the not explored list
        sorted_not_explored = sorted(self.not_explored, key=self.not_explored.get, reverse=False)

        # Get from the ordered dict, the minimum cost
        self.pos_str = str(sorted_not_explored[0])
        self.pos = self.string_to_array(self.pos_str)

        # Pop that element from the dictiionary and get its depth
    ...
    def heuristic(self, move):
        distamce = np.sqrt((move[0] - goal[0])**2 + (move[1] - goal[1])**2)
        return round(distamce, 1)
