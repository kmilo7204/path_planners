...
# Create intersections from above graph
...
# Create Connections from above graph
...
class AStarSearch:
    ...
    def get_possible_moves(self):
        ...
        for connection in self.location.connections:
            """
            The following evaluation will help to get a consistent path
                - Remember that the main idea of A* is to explore first the most
                promising path, before changing its route.
                - Currently we are not doing that, because we are finding the goal
                in the not explored list and stoping the search in that race condition.
                - Now we need to consider the case where we explore a possible movement
                and we find a new cost for a previous connection (Not explored yet, but
                added to that list). If the new cost is lower than the previous one, then
                we will replace that connection and cost.
            """
            if connection in self.not_explored:
                # Calculate the new cost of moving into the next node
                # Current depth + dsitance from my node to the next one
                self.not_explored[connection] = self.length_depth + self.road_distance(self.location, connection)
                # Intersection cost
            ...
            if connection not in self.explored and connection not in self.not_explored:
                # ---> We calculate the cost of moving in that direction
                # This point is really important because the new cost considers
                # not only the previous cost, but the distance between the location
                # and the connection, the intersection cost, and the heuristic which
                # in this case it is the time to flight
        ...
        # We consider the current depth and the heuristic
        self.explored[self.location] = self.length_depth + \
            self.heuristic(self.location)

    def goal_found(self):
        # This condition changes because here we will explored previous other
        # possible nodes with better costs to reach the goal.
        if self.goal in self.explored:
            # We will not be adding the final point to a path
            # as soon as we find the goal, we completed the job.
            return True
        return False
    ...
    def explore_next_move(self):
        ...
        # We will sort the list and we will use the one with the lowest cost
        ...
        # We need to extract the heuristic added in the exploration
        self.length_depth = self.not_explored.pop(sorted_not_explored[0]) - self.heuristic(self.location)
        return True
